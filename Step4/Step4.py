...
def compStore():
    f = open(filename, 'wb')
    pickle.dump({'Component Type:': compTypeChoice, 'Fitment Type:': fitTypeChoice, 'Date of Manufacture': dateOfMan, 'Manufacture Progress:': manProgress, 'Batch Number:': batchNo}, f)
    f.close()
...
while True:
    ...
    print('[6]Component Search')
    print('[7]Finish a Component')
    print('[8]Quit Program')
    newln()

    userChoice = input('Please Press the key 1, 2, 3, 4, 6, 7 or 8:')
    newln()
    ...
    if userChoice == '1':
        ...
        manProgress = 'MANUFACTURE-UNFINISHED'
        ...
        while Scounter < noOfComp:
                ...
                CompData = {'Component Type:': compTypeChoice, 'Fitment Type:': fitTypeChoice, 'Date of Manufacture': dateOfMan, 'Manufacture Progress:': manProgress, 'Batch Number:': batchNo}
                filename = batchNo + '-' + str(Scounter).zfill(4) + '.pck'
                compStore()
                search[batchNo] = CompData
        ...
        compSearch = input('1.Winglet Strut, 2.Door Handle, 3.Rudder Pin')
        fitSearch = input('1. A320 Series, 2. A380 Series')

        if compSearch == '1':
            compSearch = Component.componentType['1']
        elif compSearch == '2':
            compSearch = Component.componentType['2']
        elif compSearch == '3':
            compSearch = Component.componentType['3']

        if fitSearch == '1':
            fitSearch = Component.fitmentType['1']
        elif fitSearch == '2':
            fitSearch = Component.fitmentType['2']


        if search == compSearch and search == fitSearch:
            print(search)
        ...
        finishStyle = ''
        while finishStyle != '1' or finishStyle != '2':
            ...
            finishStyle = input('Will this component have a polish or paint finish? \n[1]polish \n[2]paint')

            if finishStyle == '1':
                f = open(filename, 'wb')
                manProgress = 'Polished'
                compStore()
                break
            if finishStyle == '2':
                colour = input('What colour will this component be? \n (Use a 4 digit colour code)')
                f = open(filename, 'wb')
                manProgress = colour
                compStore()
                break
